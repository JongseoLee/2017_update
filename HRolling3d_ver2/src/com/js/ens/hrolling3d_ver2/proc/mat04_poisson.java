...
public class mat04_poisson {
	...
	public void running(String type, String standType){	//Constant || Table
		...
		if(this.type.equals("constant")){
			ori_filePath = myUtil.setPath(myUtil.setPath(modulePath, standType), this.FileName_mat04Con);
			new_filePath = myUtil.setPath(myUtil.setPath(myUtil.setPath(MC.getWorkspace(), FolderTree.folderName_proc),standType),this.FileName_mat04Con);
		}else {
			ori_filePath = myUtil.setPath(myUtil.setPath(modulePath, standType), this.FileName_mat04Tbl);
			new_filePath = myUtil.setPath(myUtil.setPath(myUtil.setPath(MC.getWorkspace(), FolderTree.folderName_proc),standType),this.FileName_mat04Tbl);
		}
	...
	private void swapValue(int StandType){
		...
		for(String line : this.ori_fileDataList){
			if(line.contains(GenProcMat.PR_value)){
				newLine = line.replace(GenProcMat.PR_value, obj.getPR_Value());
				this.procDataList.add(newLine);
			}else if(line.contains(GenProcMat.PR_tableName)){
				newLine = line.replace(GenProcMat.PR_tableName, this.tableName);
				this.procDataList.add(newLine);
			}else if(line.contains(GenProcMat.PR_tableData)){
	...
	private void readMaterialTableData(String standType){
		if(standType.equals(UILabel.F1)){
			...
			Reader reader = new Reader(obj.getPR_Value_T());
			reader.running();
