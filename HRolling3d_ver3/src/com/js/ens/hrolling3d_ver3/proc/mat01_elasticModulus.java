...
public class mat01_elasticModulus {
	...
	public mat01_elasticModulus() {
		// TODO Auto-generated constructor stub
	}
	
	public void running(String type, String standType){	//Constant || Table
		this.type = type;
		...
		if(this.type.equals("constant")){
			ori_filePath = myUtil.setPath(myUtil.setPath(modulePath, standType), this.FileName_mat01Con);
			new_filePath = myUtil.setPath(myUtil.setPath(myUtil.setPath(MC.getWorkspace(),FolderTree.folderName_proc),standType),this.FileName_mat01Con);
		}else {
			ori_filePath = myUtil.setPath(myUtil.setPath(modulePath, standType), this.FileName_mat01Tbl);
			new_filePath = myUtil.setPath(myUtil.setPath(myUtil.setPath(MC.getWorkspace(), FolderTree.folderName_proc),standType),this.FileName_mat01Tbl);
		}
	...
	private void swapValue(int StandType){
		...
		for(String line : this.ori_fileDataList){
			if(line.contains(GenProcMat.YM_value)){
				newLine = line.replace(GenProcMat.YM_value, obj.getYM_Value());
				this.procDataList.add(newLine);
			}else if(line.contains(GenProcMat.YM_tableName)){
				newLine = line.replace(GenProcMat.YM_tableName, this.tableName);
				this.procDataList.add(newLine);
			}else if(line.contains(GenProcMat.YM_tableData)){
	...
	private void readMaterialTableData(String standType){
		if(standType.equals(UILabel.F1)){
			...
			Reader reader = new Reader(obj.getYM_Value_T());
			reader.running();
